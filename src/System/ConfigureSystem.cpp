...
void ConfigureSystem::SetSystemSolver()
{
	m_system->SetSolver(std::make_shared<chrono::ChSolverMINRES>());
}

void ConfigureSystem::SetupCamera()
{
	m_application->AddTypicalCamera(K_INITIAL_POSITION);

	auto* settings = AlgorithmSettings::GetInstance();
	const int oxSize = settings->GetOxSize();
	const int oySize = settings->GetOySize();
	const int ozSize = settings->GetOzSize();
	const double elementSize = settings->GetElementSize();

	irr::core::vector3df buildingCenter(
		(oxSize * elementSize) / 2.0f,
		(oySize * elementSize) / 2.0f,
		(ozSize * elementSize) / 2.0f);

	auto* camera = m_application->GetSceneManager()->getActiveCamera();
...
void ConfigureSystem::RunIrrlichtScene()
{
	...
	ImGuiEventReceiver receiver{ m_application, &m_cameraAutoRotation };
	...
	while (m_application->GetDevice()->run())
	{
		...
		if (m_cameraAutoRotation)
		{
			float angle = K_BASE_ROTATION_SPEED * m_cameraRotationSpeed;
			auto* camera = m_application->GetSceneManager()->getActiveCamera();
			CameraRotationReceiver::RotateCameraAroundTarget(camera, angle);
		}
...
void ConfigureSystem::ManageKeyStrokes()
{
...
void ConfigureSystem::DrawCameraControlsUI()
{
	...
	const float panelHeight = 110.0f;
	...
	ImGui::Begin("CameraControls", nullptr,
		ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse |
		ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoTitleBar);

	if (ImGui::Button(m_cameraAutoRotation ? "Auto Rotation: On" : "Auto Rotation: Off",
		ImVec2(panelWidth - 28.0f, 40.0f)))
	{
		m_cameraAutoRotation = !m_cameraAutoRotation;
	}

	ImGui::Spacing();
	ImGui::TextColored(textColor, "Rotation Speed");
	ImGui::SliderFloat("##RotationSpeed", &m_cameraRotationSpeed, 0.25f, 2.0f, "%.2fx");
